...
class InvoiceView {
    public static function render($header, $items) {
        ...
        $rawText .= str_pad($judul, 63, " ", STR_PAD_BOTH) . $LN . $LN;
		$rawText .= $ESC . "E" . chr(0);
        
        // Header Info
        $dateLine = "Date : " . str_pad(($header['sales_date'] ?? '-'), 53) . str_pad("Print#", 10, " ", STR_PAD_LEFT);
        $rawText .= $dateLine . $LN;
        ...
        if (!empty($items)) {
            foreach ($items as $item) {
                ...
                $nama  = str_pad(substr($item['item_name'], 0, 37), 38); 
		...
		$blokKiri = $simbolTotal . str_pad($teksTotal, 51 - strlen($simbolTotal), " ", STR_PAD_LEFT);
